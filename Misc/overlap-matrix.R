...
dryoverlap_df<- dry_fc %>%
  ...
  group_by(taxon, year) %>%
  summarise(overlap = overlap(list(prob_norm, prob_dry)))%>%
  ungroup()%>%
  mutate(plant="Dryas")
...
consumer_order <- c(
  ...
  "Ichneumonidae", "Collembola", "Sciaridae", "Phoridae", "Muscidae", "Acari"
)

matrix_data <- matrix_data %>%
  mutate(
    consumer = factor(consumer, levels = consumer_order),
    slope_color = case_when(
      slope_color == "Increasing" ~ "Increasing",
      slope_color == "Decreasing" ~ "Decreasing",
      TRUE ~ "NS"  # Replace NA with "NS"
    ),slope_color = factor(slope_color, levels = c("Increasing", "Decreasing", "NS")))

# Plot
ggplot(matrix_data, aes(x = resource, y = consumer)) +
  geom_point(
    aes(size = mean_overlap, fill = slope_color),
