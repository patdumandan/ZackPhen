...
functions {
  ...
  vector elementwise_normal(vector x, vector mu, vector si) { // This seems to speed up the model fitting, despite the looping
    vector[size(x)] probs;
    for (m in 1:size(x)) {
      probs[m] = exp(normal_lpdf(x[m] | mu[m], si[m]));
    }
    return probs;
  }
...
parameters {
  ...
  // Plot-level random intercepts, also for NCP
  ...
  real<lower=0> sigma_width; // sd of yearly peak times across years  
...
transformed parameters {
  ...
  for (n in 1:N) {
    eta[n] =alpha_year[year_id[n]] - ( DOYs[n] - mu[year_id[n]] ).^2./width[year_id[n]].^2 +
            u_plot[plot_id[n]];}
}
